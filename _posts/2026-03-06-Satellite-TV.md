---
layout: post
title: "Complete Satellite Reception Guide from Sri Lanka"
date: 2026-03-06
author: A65KJ
categories: [satellite, tv-reception, sri-lanka]
tags: [satellite-tv, fta, c-band, ku-band, card-sharing, hellobox, tiger, starsat]
description: "Comprehensive database of 30+ TV satellites receivable from Sri Lanka with orbital positions, FTA availability, encryption status, and card-sharing server compatibility"
---

*A comprehensive database of geostationary satellites visible from the Pearl of the Indian Ocean*

![Satellite Arc Visualization](/assets/images/sri_lanka_satellite_arc.png)
*Geostationary arc visible from Sri Lanka (40°E - 140°E). Color coding: Green = FTA Available, Orange = Encrypted (IKS/CS supported), Blue = Encrypted (No CS available)*

## Introduction

Living in Sri Lanka puts us in a unique position to receive signals from a wide arc of geostationary satellites spanning from **38°E to 138°E**. This guide documents my personal reception experiments using various setups including standard DVB-S2 receivers, card-sharing capable boxes like **HelloBox (Scam+ server)**, **Tiger/Starsat (Forever IKS)**, and **Funcam server** compatible devices.

**Key:**
- ✅ **FTA** = Free-to-air channels available
- 🔓 **Forever** = Forever IKS server compatible (Tiger/Starsat)
- 🔓 **Scam+** = Scam+ server compatible (HelloBox)
- 📡 **C-Band** = Requires 1.8m+ dish typically
- 📡 **Ku-Band** = Receivable on 60cm-90cm dish

---

## 🌏 Eastern Arc (100°E - 138°E)

### [Telstar 18 Vantage](https://www.lyngsat.com/Telstar-18-Vantage.html) @ 138.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 138.0°E |
| **Band** | C-Band |
| **FTA** | ❌ No |
| **Encryption** | Various |
| **CS/IKS** | Not confirmed |
| **Dish Size** | 1.8m+ |

**Description:** The easternmost satellite in this list, Telstar 18 Vantage primarily serves East Asian markets including China, Japan, and Korea. From Sri Lanka, this requires a clear eastern horizon and a larger dish due to the low elevation angle. Limited South Asian content, mainly Chinese and international channels. Signal strength is marginal from our location.

---

### [Apstar 6C](https://www.lyngsat.com/Apstar-6C.html) @ 134.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 134.0°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **Notable** | Chinese channels, some FTA |
| **Dish Size** | 1.8m+ |

**Description:** Replacement for Apstar 6, this satellite carries a mix of Chinese provincial channels and international feeds. Good for Chinese language content and some international news feeds. Reception from Sri Lanka is challenging due to the extreme eastern position but possible with a large dish and clear horizon.

---

### [LaoSat 1](https://www.lyngsat.com/LaoSat-1.html) @ 128.5°E

| Parameter | Details |
|-----------|---------|
| **Position** | 128.5°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **Content** | Lao, Thai, international |
| **Dish Size** | 1.8m+ |

**Description:** The national satellite of Laos carries Lao state TV channels and some Thai content. Limited appeal for general viewing but useful for regional language content. Signal coverage extends well into South Asia.

---

### [ChinaSat 6D](https://www.lyngsat.com/ChinaSat-6D.html) @ 125.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 125.0°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **Content** | Chinese channels |
| **Dish Size** | 1.8m+ |

**Description:** Part of the ChinaSat constellation serving mainland China. Carries numerous Chinese provincial channels in FTA. Good for Chinese language learners or those interested in Chinese media. Reception quality varies based on weather conditions.

---

### [AsiaSat 9](https://www.lyngsat.com/AsiaSat-9.html) @ 122.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 122.0°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **Notable** | Multiple FTA feeds |
| **Dish Size** | 1.8m+ |

**Description:** Successor to AsiaSat 4, this satellite hosts a variety of international and regional channels. Good FTA selection including news and entertainment channels targeting South and Southeast Asia. Strong signal footprint over Sri Lanka.

---

### [AsiaSat 6 / Thaicom 7](https://www.lyngsat.com/AsiaSat-6.html) @ 120.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 120.0°E |
| **Band** | C-Band |
| **FTA** | ❌ No |
| **Encryption** | Various |
| **Dish Size** | 1.8m+ |

**Description:** Shared satellite position with Thaicom 7. Primarily carries encrypted international channels and some occasional feeds. Limited FTA value but worth monitoring for occasional open feeds during sporting events or breaking news.

---

### [Thaicom 4 / BBH 1](https://www.lyngsat.com/Thaicom-4.html) @ 119.3°E

| Parameter | Details |
|-----------|---------|
| **Position** | 119.3°E |
| **Band** | C-Band |
| **FTA** | ❌ No |
| **Encryption** | Conax, others |
| **Dish Size** | 1.8m+ |

**Description:** Also known as IPSTAR, this satellite is primarily used for broadband services but carries some TV channels. Mostly encrypted Thai content. Limited interest for FTA enthusiasts but part of a complete arc scan.

---

### [ChinaSat 6E](https://www.lyngsat.com/ChinaSat-6E.html) @ 115.5°E

| Parameter | Details |
|-----------|---------|
| **Position** | 115.5°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **Content** | Chinese channels |
| **Dish Size** | 1.8m+ |

**Description:** Another Chinese satellite with numerous FTA channels. Similar to ChinaSat 6D but with slightly different channel lineup. Good for Chinese content enthusiasts.

---

### [ChinaSat 10](https://www.lyngsat.com/ChinaSat-10.html) @ 110.5°E

| Parameter | Details |
|-----------|---------|
| **Position** | 110.5°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **Content** | Chinese, some international |
| **Dish Size** | 1.8m+ |

**Description:** Carries Chinese CCTV channels and various provincial broadcasts. Also hosts some international channels. Strong FTA presence makes it worthwhile for those tracking Chinese media.

---

### [SES 7 / Telkom 4](https://www.lyngsat.com/SES-7.html) @ 108.1°E

| Parameter | Details |
|-----------|---------|
| **Position** | 108.1°E |
| **Band** | Ku-Band |
| **FTA** | ❌ No |
| **Encryption** | Various |
| **CS/IKS** | 🔓 F&S Server |
| **Provider** | Airtel TV |
| **Dish Size** | 90cm+ |

**Description:** This Ku-band satellite carries **Airtel TV** services for the Indian subcontinent. While encrypted, it\\'s accessible via **Funcam (F&S) server** on compatible boxes. Good selection of Indian entertainment and sports channels. Being Ku-band, it requires a smaller dish than C-band satellites.

---

## 🎯 Prime South Asian Arc (90°E - 105°E)

### [AsiaSat 7](https://www.lyngsat.com/AsiaSat-7.html) @ 105.5°E ⭐

| Parameter | Details |
|-----------|---------|
| **Position** | 105.5°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **CS/IKS** | 🔓 Forever Server |
| **Notable** | Major FTA hub |
| **Dish Size** | 1.8m+ |

**Description:** One of the **most important satellites** for South Asian FTA viewing! AsiaSat 7 hosts a massive collection of channels including:
- **Star TV** package (regional languages)
- **Zee Network** channels
- **Sony** channels
- **Pakistani channels** (ARY, Geo, etc.)
- **International news** (CNN, BBC, Al Jazeera)
- **Religious channels**

Many channels are FTA, while others work with **Forever IKS server**. This is a must-have satellite for any serious satellite enthusiast in Sri Lanka. The C-band signal is strong and stable.

---

### [Express AMU3](https://www.lyngsat.com/Express-AMU3.html) @ 103.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 103.0°E |
| **Band** | C-Band |
| **FTA** | ❌ No |
| **Encryption** | Various |
| **Dish Size** | 1.8m+ |

**Description:** Russian satellite with limited South Asian footprint. Primarily carries Russian and Central Asian content. Not particularly useful for general viewing in Sri Lanka but completes the arc scan.

---

### [AsiaSat 5](https://www.lyngsat.com/AsiaSat-5.html) @ 100.5°E

| Parameter | Details |
|-----------|---------|
| **Position** | 100.5°E |
| **Band** | C/Ku-Band |
| **FTA** | ✅ Yes |
| **Notable** | C-Band feeds |
| **Dish Size** | C: 1.8m+, Ku: 90cm |

**Description:** Dual-band satellite with interesting **C-band feeds** that occasionally carry live events, sports feeds, and news backhauls. The Ku-band side has some DTH services. Worth monitoring for occasional interesting feeds.

---

### [SES 8 / SES 12](https://www.lyngsat.com/SES-8.html) @ 95.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 95.0°E |
| **Band** | Ku-Band |
| **FTA** | ❌ No |
| **Encryption** | Various |
| **Provider** | Dish TV India |
| **Dish Size** | 90cm+ |

**Description:** Carries **Dish TV India** services. Ku-band only, so manageable dish size. Encrypted service, currently no reliable card-sharing support documented. Good signal strength over Sri Lanka.

---

### [G-Sat 15 / G-Sat 17](https://www.lyngsat.com/G-Sat-15.html) @ 93.5°E

| Parameter | Details |
|-----------|---------|
| **Position** | 93.5°E |
| **Band** | C/Ku-Band |
| **FTA** | ✅ Yes |
| **Notable** | DD Free Dish |
| **Provider** | Dish TV India & DD Free Dish |
| **Dish Size** | C: 1.8m+, Ku: 90cm |

**Description:** Indian national satellites carrying **DD Free Dish** (India\\'s free DTH service) and **Dish TV** encrypted services. Excellent FTA selection including all Doordarshan channels, Indian news, movies, and regional content. The **DD Free Dish** service alone makes this satellite essential. Strong signals on both bands.

---

### [Measat 3B / Measat 3D](https://www.lyngsat.com/Measat-3B.html) @ 91.5°E ⭐

| Parameter | Details |
|-----------|---------|
| **Position** | 91.5°E |
| **Band** | C/Ku-Band |
| **FTA** | ❌ No |
| **CS/IKS** | 🔓 Forever Server |
| **Provider** | Sun Direct |
| **Dish Size** | C: 1.8m+, Ku: 90cm |

**Description:** Malaysian satellite that hosts **Sun Direct** DTH service for South India and Sri Lanka. While encrypted, it works with **Forever IKS server**. Excellent for Tamil, Telugu, Malayalam, and Kannada content. The Ku-band signal is particularly strong in Sri Lanka.

---

### [ST 2](https://www.lyngsat.com/ST-2.html) @ 88.0°E ⭐

| Parameter | Details |
|-----------|---------|
| **Position** | 88.0°E |
| **Band** | C/Ku-Band |
| **FTA** | ✅ Yes |
| **CS/IKS** | 🔓 Scam+ Server |
| **Provider** | Videocon D2H |
| **Dish Size** | C: 1.8m+, Ku: 90cm |

**Description:** Singapore-based satellite carrying **Videocon D2H** services. Has both encrypted (accessible via **Scam+ server** on HelloBox) and some FTA channels. Good mix of Hindi and regional Indian content. Strong signal footprint over Sri Lanka.

---

### [G-Sat 10 / G-Sat 24 / G-Sat 30](https://www.lyngsat.com/G-Sat-10.html) @ 83.0°E ⭐

| Parameter | Details |
|-----------|---------|
| **Position** | 83.0°E |
| **Band** | C/Ku-Band |
| **FTA** | ❌ No |
| **CS/IKS** | 🔓 Forever Server |
| **Provider** | Tata Sky / Tata Play |
| **Dish Size** | C: 1.8m+, Ku: 90cm |

**Description:** One of the **most important satellites** for premium Indian content! Carries **Tata Sky** (now Tata Play), India\\'s largest DTH platform. Accessible via **Forever IKS server**. Massive channel selection including:
- All major Hindi entertainment
- Premium sports (Star Sports, Sony Sports)
- International channels
- HD channels

Being only 3° east of Sri Lanka\\'s longitude, this satellite has excellent elevation and signal strength.

---

## 🏠 Central Arc (75°E - 83°E)

### [Express 80](https://www.lyngsat.com/Express-80.html) @ 80.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 80.0°E |
| **Band** | Ku-Band |
| **FTA** | ✅ Yes |
| **Dish Size** | 90cm+ |

**Description:** Russian satellite at nearly the same longitude as Sri Lanka! This means **maximum elevation angle** (almost overhead), making it ideal for locations with limited horizon views. Carries some FTA channels including Russian international channels and occasional feeds.

---

### [Thaicom 6 / Thaicom 8](https://www.lyngsat.com/Thaicom-6.html) @ 78.5°E

| Parameter | Details |
|-----------|---------|
| **Position** | 78.5°E |
| **Band** | C/Ku-Band |
| **FTA** | ❌ No |
| **Encryption** | Various |
| **Provider** | TrueVisions |
| **Dish Size** | C: 1.8m+, Ku: 90cm |
| **Notes** | 6ft dish recommended |

**Description:** Thai national satellite carrying **TrueVisions** DTH service. Thai content primarily. The note about requiring a 6ft dish suggests signal is weaker or tightly beamed. Interesting for Thai content enthusiasts but limited general appeal.

---

### [Apstar 7](https://www.lyngsat.com/Apstar-7.html) @ 76.5°E ⭐

| Parameter | Details |
|-----------|---------|
| **Position** | 76.5°E |
| **Band** | C/Ku-Band |
| **FTA** | ✅ Yes |
| **CS/IKS** | 🔓 Forever Server |
| **Provider** | Sky Net (Myanmar) |
| **Dish Size** | C: 1.8m+, Ku: 90cm |
| **Notes** | 6ft dish recommended |

**Description:** Important satellite for Myanmar and Southeast Asian content. Carries **Sky Net** DTH service accessible via **Forever IKS**. Also has FTA channels. The 6ft dish recommendation suggests tight beam focus, but content includes Myanmar channels and some international feeds.

---

### [ABS 2 / ABS 2A](https://www.lyngsat.com/ABS-2.html) @ 75.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 75.0°E |
| **Band** | C/Ku-Band |
| **FTA** | ✅ Yes |
| **Notable** | Multi-region coverage |
| **Dish Size** | C: 1.8m+, Ku: 90cm |

**Description:** Hong Kong-based satellite with broad coverage from Middle East to Asia. Good FTA selection including religious channels, international news, and regional content. Strong signal over South Asia.

---

### [G-Sat 18](https://www.lyngsat.com/G-Sat-18.html) @ 74.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 74.0°E |
| **Band** | C/Ku-Band |
| **FTA** | ✅ Yes |
| **Content** | Indian channels |
| **Dish Size** | C: 1.8m+, Ku: 90cm |

**Description:** Indian communication satellite with TV broadcasting capabilities. Carries various Indian channels, some FTA. Good addition to the Indian satellite cluster.

---

### [Eutelsat 70B](https://www.lyngsat.com/Eutelsat-70B.html) @ 70.5°E

| Parameter | Details |
|-----------|---------|
| **Position** | 70.5°E |
| **Band** | Ku-Band |
| **FTA** | ❌ No |
| **Encryption** | Various |
| **Provider** | Dish TV India |
| **Dish Size** | 90cm+ |

**Description:** European satellite operator\\'s presence in the Indian Ocean region. Carries some Dish TV India services. Ku-band only.

---

## 🌍 Western Arc (38°E - 68°E)

### [Intelsat 20 / Intelsat 36](https://www.lyngsat.com/Intelsat-20.html) @ 68.5°E ⭐

| Parameter | Details |
|-----------|---------|
| **Position** | 68.5°E |
| **Band** | C-Band |
| **FTA** | ❌ No |
| **CS/IKS** | 🔓 Forever Server |
| **Dish Size** | 1.8m+ |

**Description:** Major satellite for African and Indian Ocean region. While primarily encrypted, accessible via **Forever IKS server**. Carries various international and African channels. Good signal strength in Sri Lanka.

---

### [Intelsat 17](https://www.lyngsat.com/Intelsat-17.html) @ 66.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 66.0°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **Notable** | African/Asian beams |
| **Dish Size** | 1.8m+ |

**Description:** Covers Africa, Middle East, and Indian subcontinent. Good FTA selection including religious channels and international broadcasters. Strong footprint over Sri Lanka.

---

### [NSS 12](https://www.lyngsat.com/NSS-12.html) @ 57.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 57.0°E |
| **Band** | C/Ku-Band |
| **FTA** | ❌ No |
| **Encryption** | Various |
| **Dish Size** | C: 1.8m+, Ku: 90cm |

**Description:** European satellite serving Africa and Asia. Primarily carries data and encrypted TV services. Limited FTA value but part of a complete setup.

---

### [Intelsat 38](https://www.lyngsat.com/Intelsat-38.html) @ 45.0°E ⭐

| Parameter | Details |
|-----------|---------|
| **Position** | 45.0°E |
| **Band** | Ku-Band |
| **FTA** | ❌ No |
| **CS/IKS** | 🔓 F&S Server |
| **Provider** | Dialog TV (Sri Lanka) |
| **Dish Size** | 90cm+ |

**Description:** **Sri Lanka\\'s primary DTH satellite!** Intelsat 38 carries **Dialog TV**, the country\\'s largest satellite TV provider. Accessible via **Funcam (F&S) server** on compatible boxes. This is the most important satellite for local content including:
- All local Sri Lankan channels
- Indian channels
- International sports
- Movies

Being Ku-band, it requires only a 90cm dish and has excellent signal strength nationwide.

---

### [Express AM7](https://www.lyngsat.com/Express-AM7.html) @ 40.0°E

| Parameter | Details |
|-----------|---------|
| **Position** | 40.0°E |
| **Band** | Ku-Band |
| **FTA** | ✅ Yes |
| **Provider** | Freesat (UK) |
| **Dish Size** | 90cm+ |

**Description:** Russian satellite that also carries **Freesat** UK service (though UK reception requires local dishes). From Sri Lanka, some transponders may be receivable with interesting international content. Challenging due to western position but possible.

---

### [Paksat 1R](https://www.lyngsat.com/Paksat-1R.html) @ 38.0°E ⭐

| Parameter | Details |
|-----------|---------|
| **Position** | 38.0°E |
| **Band** | C-Band |
| **FTA** | ✅ Yes |
| **CS/IKS** | 🔓 Forever Server |
| **Content** | Pakistani channels |
| **Dish Size** | 1.8m+ |

**Description:** Pakistan\\'s national satellite and one of the **best sources of FTA content**! Carries extensive Pakistani channel lineup including:
- **PTV** channels
- **ARY** channels (FTA versions)
- **Geo** channels (FTA versions)
- **Hum TV** (FTA)
- **Masala TV** (FTA)
- Numerous news and entertainment channels

Many channels are FTA, and some encrypted ones work with **Forever IKS**. Essential for Urdu/Hindi content enthusiasts. This is the westernmost satellite in the list, requiring a clear western horizon.

---

## 📊 Quick Reference Summary

| Satellite | Position | Band | FTA | CCCAM | Priority |
|-----------|----------|------|-----|--------|----------|
| [Telstar 18](https://www.lyngsat.com/Telstar-18-Vantage.html) | 138.0°E | C | ❌ | - | Low |
| [Apstar 6C](https://www.lyngsat.com/Apstar-6C.html) | 134.0°E | C | ✅ | - | Medium |
| [LaoSat 1](https://www.lyngsat.com/LaoSat-1.html) | 128.5°E | C | ✅ | - | Low |
| [ChinaSat 6D](https://www.lyngsat.com/ChinaSat-6D.html) | 125.0°E | C | ✅ | - | Medium |
| [AsiaSat 9](https://www.lyngsat.com/AsiaSat-9.html) | 122.0°E | C | ✅ | - | High |
| [AsiaSat 6/Thaicom 7](https://www.lyngsat.com/AsiaSat-6.html) | 120.0°E | C | ❌ | - | Low |
| [Thaicom 4/BBH 1](https://www.lyngsat.com/Thaicom-4.html) | 119.3°E | C | ❌ | - | Low |
| [ChinaSat 6E](https://www.lyngsat.com/ChinaSat-6E.html) | 115.5°E | C | ✅ | - | Medium |
| [ChinaSat 10](https://www.lyngsat.com/ChinaSat-10.html) | 110.5°E | C | ✅ | - | Medium |
| [SES 7/9/Telkom 4](https://www.lyngsat.com/SES-7.html) | 108.1°E | Ku | ❌ | Forever & Scam+ | High |
| [AsiaSat 7](https://www.lyngsat.com/AsiaSat-7.html) ⭐ | 105.5°E | C | ✅ | Forever | **Essential** |
| [Express AMU3](https://www.lyngsat.com/Express-AMU3.html) | 103.0°E | C | ❌ | - | Low |
| [AsiaSat 5](https://www.lyngsat.com/AsiaSat-5.html) | 100.5°E | C/Ku | ✅ | - | Medium |
| [SES 8/12](https://www.lyngsat.com/SES-8.html) | 95.0°E | Ku | ❌ | - | Medium |
| [G-Sat 15/17](https://www.lyngsat.com/G-Sat-15.html) | 93.5°E | C/Ku | ✅ | - | **Essential** |
| [Measat 3B/3D](https://www.lyngsat.com/Measat-3B.html) ⭐ | 91.5°E | C/Ku | ❌ | Forever | High |
| [ST 2](https://www.lyngsat.com/ST-2.html) ⭐ | 88.0°E | C/Ku | ✅ | Scam+ | High |
| [G-Sat 10/24/30](https://www.lyngsat.com/G-Sat-10.html) ⭐ | 83.0°E | C/Ku | ❌ | Forever | **Essential** |
| [Express 80](https://www.lyngsat.com/Express-80.html) | 80.0°E | Ku | ✅ | - | Medium |
| [Thaicom 6/8](https://www.lyngsat.com/Thaicom-6.html) | 78.5°E | C/Ku | ❌ | - | Medium |
| [Apstar 7](https://www.lyngsat.com/Apstar-7.html) ⭐ | 76.5°E | C/Ku | ✅ | Forever | High |
| [ABS 2](https://www.lyngsat.com/ABS-2.html) | 75.0°E | C/Ku | ✅ | - | Medium |
| [G-Sat 18](https://www.lyngsat.com/G-Sat-18.html) | 74.0°E | C/Ku | ✅ | - | Medium |
| [Eutelsat 70B](https://www.lyngsat.com/Eutelsat-70B.html) | 70.5°E | Ku | ❌ | - | Low |
| [Intelsat 20/36](https://www.lyngsat.com/Intelsat-20.html) ⭐ | 68.5°E | C | ❌ | Forever | High |
| [Intelsat 17](https://www.lyngsat.com/Intelsat-17.html) | 66.0°E | C | ✅ | - | Medium |
| [NSS 12](https://www.lyngsat.com/NSS-12.html) | 57.0°E | C/Ku | ❌ | - | Low |
| [Intelsat 38](https://www.lyngsat.com/Intelsat-38.html) ⭐ | 45.0°E | Ku | ❌ | Forever & Scam+ | **Essential** |
| [Express AM7](https://www.lyngsat.com/Express-AM7.html) | 40.0°E | Ku | ✅ | - | Medium |
| [Paksat 1R](https://www.lyngsat.com/Paksat-1R.html) ⭐ | 38.0°E | C | ✅ | Forever | **Essential** |

---

## 🎯 Recommended Setup for Sri Lanka

### Essential Satellites (Must-Have)
1. **[Intelsat 38](https://www.lyngsat.com/Intelsat-38.html) @ 45.0°E** - Dialog TV (local content)
2. **[G-Sat 10/24/30](https://www.lyngsat.com/G-Sat-10.html) @ 83.0°E** - Tata Sky (premium Indian)
3. **[AsiaSat 7](https://www.lyngsat.com/AsiaSat-7.html) @ 105.5°E** - Major FTA hub
4. **[G-Sat 15/17](https://www.lyngsat.com/G-Sat-15.html) @ 93.5°E** - DD Free Dish (FTA Indian)
5. **[Paksat 1R](https://www.lyngsat.com/Paksat-1R.html) @ 38.0°E** - Pakistani FTA

### High Value Additions
- **[Measat 3B/3D](https://www.lyngsat.com/Measat-3B.html) @ 91.5°E** - Sun Direct (South Indian)
- **[ST 2](https://www.lyngsat.com/ST-2.html) @ 88.0°E** - Videocon D2H
- **[Apstar 7](https://www.lyngsat.com/Apstar-7.html) @ 76.5°E** - Sky Net (Myanmar)
- **[Intelsat 20/36](https://www.lyngsat.com/Intelsat-20.html) @ 68.5°E** - African/International
- **[SES 7/9](https://www.lyngsat.com/SES-7.html) @ 108.1°E** - Airtel TV

### Equipment Recommendations

**For Ku-Band (Dialog, Tata Sky, etc.):**
- 90cm - 1.2m offset dish
- Universal LNB (9.75/10.6 GHz)

**For C-Band (AsiaSat, Paksat, etc.):**
- 1.8m - 2.4m prime focus dish
- C-Band LNB (3.4-4.2 GHz)

**Recommended Receivers:**
- **HelloBox** - Scam+ server support
- **Tiger/Starsat/Mediastar** - Forever server support
- **Funcam compatible boxes**

---

## ⚠️ Legal Notice

*Card sharing and IKS (Internet Key Sharing) exist in legal gray areas in many jurisdictions. This guide is for educational purposes only. Please ensure you comply with local laws and service provider terms of service. FTA (Free-to-Air) channels are legal to receive without subscription.*

---

## 🔗 External Resources

- [Lyngsat](https://www.lyngsat.com) - Comprehensive satellite channel database
- [SatBeams](https://www.satbeams.com) - Satellite footprint maps
- [Flysat](https://www.flysat.com) - Satellite updates and news
- [KingOfSat](https://www.kingofsat.net) - European satellite information

---


*73 de A65KJ* 📡

